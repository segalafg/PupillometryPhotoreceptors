...
pdf(paste0(figdir,"methodsfigrods1.pdf"), bg="transparent", height = 13, width = 12)
...
To quantitatively summarise our data, we used the same model described in a previous study [@Segala2023]. The model has the same general form as the first stage of the contrast gain control model proposed by Meese and colleagues [@Meese2006], but omits the second stage. For the previous model that was used by Segala and colleagues [@Segala2023], the exponent of the numerator and denominator had fixed values of 2 and (implicitly) 1. Here, we allow these parameters (called _p_ and _q_) to be free, in order to permit different shapes of contrast response function, e.g. accelerating or saturating. The responses of the left eye and right eye channels are as follows:
